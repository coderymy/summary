...
> 1. 一个分片就是一个lucene索引
> 2. 其中存储着包含倒排索引的文件目录
> 3. 一个索引有很多分片，一个分片只属于一个索引
> 4. 一旦索引创建，分片的数量不可变
>
> 索引到一个分片上，会分发到所有分片上获取数据，所有分片数据返回之后才会返回

4、什么是分段

> 分片中的一个一个空间，同一时间只打开一个分段，且只能向打开的分段中写入数据。分段数量超过设置的数量，需要进行**分段合并**。
>
> 写入只往打开的分段写入。读取只往关闭的分段读取，所以是准实时。

5、match和term的区别

> match会对关键字进行分词，然后根据分词的所有词汇进行搜索
>
> term不会对关键字进行分词，会直接使用关键字获取数据
