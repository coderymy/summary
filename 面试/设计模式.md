...
[学习自捡田螺的小男孩—设计模式](https://mp.weixin.qq.com/s/5OSKWsesOoottffNHEDBWQ)



# 策略模式

不同的小姐姐约会方式不一样，有的喜欢逛街，有的喜欢看电影，有的喜欢吃饭。根据小姐姐的名字来选择不同的策略哄小姐姐开心

**代码的实现逻辑**：

> 一个接口定义小姐姐基本属性、适合小姐姐的行为
>
> 多个实现类（多个小姐姐）
>
> 创建Map将小姐姐适合的方式都放到Map里面。具体做什么，就看和哪个小姐姐

**适用场景**

对于有很多会根据一个特性判断下面逻辑走向的代码。比如说**根据天气情况选择穿搭**，**根据省市区选择口味**等等



# 责任链模式

就是顺序的进行一类业务逻辑的处理。比如校验接口，需要校验参数、校验token、校验数据合法性、校验订单是否存在、校验订单状态是否正常。

责任链模式为请求创建了一个接收者对象的链。执行链上有多个对象节点，每个对象节点都有机会（条件匹配）处理请求事务，如果某个对象节点处理完了，就可以**根据实际业务需求传递给下一个节点继续处理或者返回处理完毕**。这种模式给予请求的类型，对请求的发送者和接收者进行解耦。

![](https://coderymy-image.oss-cn-beijing.aliyuncs.com/picgo/20230208144140.png)

![](https://coderymy-image.oss-cn-beijing.aliyuncs.com/picgo/20230208145728.png)
